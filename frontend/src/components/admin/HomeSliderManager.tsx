...
import { cmsApi, mediaApi } from '@api/endpoints';
...
export default function HomeSliderManager() {
  const { user } = useAuth();
  const isDev = user?.role === 'super_admin' || user?.role === 'admin';
  ...
  const loadData = async () => {
    try {
      ...
      showToast('Error', 'Failed to load slider data', 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      {loading ? (
        <div className="animate-pulse bg-white rounded-3xl p-8 border border-gray-100 h-64 flex items-center justify-center">
          <span className="text-gray-400 font-bold">Loading slides...</span>
        </div>
      ) : slides.length === 0 ? (
        <div className="bg-white rounded-3xl p-12 border border-gray-100 text-center shadow-sm">
          ...
          <p className="text-gray-500 mb-6">Create your first slide to display on the homepage hero section.</p>
          {isDev && (
            <button onClick={openNewSlideModal} className="bg-primary-100 text-primary-700 hover:bg-primary-200 font-bold px-6 py-3 rounded-xl transition-colors">
              Create First Slide
            </button>
          )}
        </div>
          ...
          {slides.map((slide, index) => (
            <div key={index} className="bg-white rounded-3xl overflow-hidden border border-gray-200 shadow-sm hover:shadow-xl transition-shadow group flex flex-col">
              ...
              <div className="p-5 flex-1 flex flex-col">
                <div className="text-xs font-bold text-primary-600 uppercase tracking-wider mb-1">{slide.badge}</div>
                <h3 className="font-extrabold text-gray-900 text-lg mb-2 line-clamp-2">{slide.heading}</h3>
                        ...
                        <button
                          ...
                          onClick={() => {
                            setConfirmDialog({
                              visible: true,
                              title: 'Remove Image',
                              message: 'Are you sure you want to remove this background image?',
                              type: 'danger',
                              onConfirm: () => {
                      ...
                      {editingSlide.logos?.map((logoUrl: string, i: number) => (
                        <div key={i} className="relative group w-14 h-14 bg-white rounded-xl shadow-sm border border-gray-200 flex items-center justify-center p-1.5">
                          ...
                          <button
                            ...
                            className="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 shadow-md transition-all z-10"
                            title="Remove Logo"
                          >
                            🗑️
                          </button>
