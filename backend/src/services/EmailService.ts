...
export class EmailService {
    ...
    if (!EmailService.transporter) {
      const transportConfig: any = {
        ...
        auth: {
          ...
          pass: process.env.SMTP_PASS || process.env.SMTP_PASSWORD || 'your_app_password',
        },
      };
  ...
  static async sendMail(options: {
    ...
    const fromAddress = process.env.FROM_EMAIL || process.env.SMTP_FROM || 'noreply@collegeerp.com';
    const fromName = process.env.SMTP_FROM_NAME || process.env.APP_NAME || 'EIILM College';
    ...
    try {
      ...
      const info = await transporter.sendMail({
        ...
        text: options.text || `${fromName} Notification`,
        html: options.html,
      });
  ...
  // 5. Account Setup Email Template
  static async sendAccountSetupEmail(email: string, details: { fullName: string; setupUrl: string }) {
    const htmlContent = `
      <div style="font-family: 'Inter', system-ui, -apple-system, sans-serif; max-width: 550px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.05);">
        <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; padding: 32px 24px; text-align: center;">
          <h2 style="margin: 0; font-size: 22px; font-weight: 700; letter-spacing: -0.025em;">Welcome to ${process.env.APP_NAME || 'EIILM College'}</h2>
          <p style="margin: 8px 0 0 0; opacity: 0.9; font-size: 14px;">Set up your account</p>
        </div>
        <div style="padding: 32px 24px; color: #1e293b; line-height: 1.6; background-color: #ffffff;">
          <p style="margin-top: 0; font-size: 16px;">Hello <strong>${details.fullName}</strong>,</p>
          <p style="font-size: 15px; color: #475569;">An administrator has created an account for you at ${process.env.APP_NAME || 'EIILM College'}. Please set your password to activate your account.</p>
          
          <div style="text-align: center; margin: 32px 0;">
            <a href="${details.setupUrl}" style="background-color: #10b981; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block; font-size: 16px; transition: background-color 0.2s;">Set Password</a>
            <br /><br />
            <span style="font-size: 12px; color: #64748b; word-break: break-all;">Setup Link: ${details.setupUrl}</span>
          </div>

          <p style="font-size: 14px; color: #64748b; margin-bottom: 0;">This link is valid for 24 hours. If the link expires, you can request a new one by contacting your administrator.</p>
          
          <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 32px 0 24px 0;" />
          <div style="text-align: center;">
            <p style="font-size: 11px; color: #94a3b8; margin: 0;">This is an automated notification from ${process.env.APP_NAME || 'EIILM College'}.</p>
          </div>
        </div>
      </div>
    `;
    ...
    return EmailService.sendMail({
      to: email,
      subject: `Welcome to ${process.env.APP_NAME || 'EIILM College'} - Set up your account`,
      html: htmlContent
    });
  }
}
